...
export class UsersService {
  ...
  /**
   * Find all users with plan information (admin)
   */
  async findAllWithPlans(filter: UserFilterDto): Promise<{
    data: any[];
    total: number;
    page: number;
    limit: number;
    ...
    // Calculate trial days (7 days from account creation)
    const trialDays = 7;
    ...
    const usersWithPlans = result.data.map(user => {
      ...
      if (subscription) {
        ...
        subscriptionInfo = {
          ...
          planName: planNames[subscription.planType],
          dailyLimit: PLAN_DAILY_LIMITS[subscription.planType],
        ...
        if (trialDaysRemaining > 0) {
          ...
          subscriptionInfo = {
            ...
            planName: 'Gratis (Trial)',
            dailyLimit: PLAN_DAILY_LIMITS[SubscriptionPlan.FREE],
            status: 'trial',
            trialDaysRemaining,
          };
        } else {
