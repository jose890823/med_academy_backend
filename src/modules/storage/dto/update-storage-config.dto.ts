...
export class S3ConfigDto {
  ...
  @ApiProperty({ example: 'AKIAIOSFODNN7EXAMPLE', description: 'Access Key ID' })
...
export class GCSConfigDto {
  @ApiProperty({ example: 'my-project-id', description: 'ID del proyecto de GCP' })
  ...
  @ApiPropertyOptional({
    description: 'Credenciales de cuenta de servicio (alternativa a keyFilename)',
  })
  @IsOptional()
  @ValidateNested()
...
export class CloudinaryConfigDto {
  ...
  @ApiProperty({ example: 'abcdefghijklmnopqrstuvwxyz', description: 'API Secret' })
  @IsNotEmpty({ message: 'El API Secret es obligatorio' })
  @IsString()
  apiSecret: string;

  @ApiPropertyOptional({
    example: 'ultrasound-academy',
