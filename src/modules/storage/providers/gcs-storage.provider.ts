...
export class GCSStorageProvider implements IStorageProvider {
  ...
  async initialize(providerConfig: StorageProviderConfig): Promise<void> {
    ...
    this.logger.log(`GCSStorageProvider initialized for bucket: ${config.bucket}`);
  }
  ...
  async upload(file: Buffer | Readable, options: UploadOptions): Promise<StorageResult> {
    ...
    const filename = options.filename || this.generateFilename(options.mimeType);
    ...
    const result: StorageResult = {
      path: key,
      url: options.isPublic ? this.getPublicUrl(key) : await this.getSignedUrl(key),
  ...
  private uploadStream(stream: Readable, gcsFile: File, options: any): Promise<void> {
