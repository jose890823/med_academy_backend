...
export class CloudinaryStorageProvider implements IStorageProvider {
  ...
  async initialize(providerConfig: StorageProviderConfig): Promise<void> {
    ...
    this.logger.log(`CloudinaryStorageProvider initialized for cloud: ${config.cloudName}`);
  }

  /**
   * Subir un archivo
   */
  async upload(file: Buffer | Readable, options: UploadOptions): Promise<StorageResult> {
  ...
  async move(sourcePath: string, destinationPath: string): Promise<StorageResult> {
    ...
    const result = await cloudinary.uploader.rename(sourcePath, destinationPath, {
      resource_type: 'auto',
      overwrite: true,
    });
  ...
  async list(
    ...
    do {
      const result = await cloudinary.api.resources({
        ...
        max_results: options?.limit ? Math.min(options.limit - files.length, 500) : 500,
   ...
   */
  async getUsageInfo(): Promise<{ used: number; total?: number; available?: number }> {
    this.ensureInitialized();
    ...
    return {
      ...
      available: usage.storage?.limit ? usage.storage.limit - usage.storage.usage : undefined,
    };
  }
  ...
  private uploadStream(stream: Readable, options: UploadApiOptions): Promise<UploadApiResponse> {
    return new Promise((resolve, reject) => {
      const uploadStream = cloudinary.uploader.upload_stream(options, (error, result) => {
        if (error) reject(error);
        else if (result) resolve(result);
