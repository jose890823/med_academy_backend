...
export class StorageConfigService implements OnModuleInit {
   ...
   */
  async activateProvider(provider: StorageProviderType): Promise<StorageConfig> {
    const config = await this.getConfig(provider);
   ...
   */
  async getProviderConfig(provider: StorageProviderType): Promise<StorageProviderConfig> {
    const config = await this.getConfig(provider);

    // Desencriptar campos sensibles
    const decryptedConfig = this.decryptSensitiveFields(config.config, provider);
